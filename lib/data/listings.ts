...
export const getListingBySlug = cache(async (slug: string) => {
  ...
  const { data } = await supabase
    .from("listings")
    .select(
      "id, title, description, price, price_negotiable, updated_at, view_count, contact_click_count, categories(name_ar, slug), neighborhoods(name_ar, slug), sellers(id, business_name, whatsapp_number, slug), listing_images(storage_path, is_primary, sort_order)"
    )
    .eq("slug", slug)
    .eq("status", "published")
