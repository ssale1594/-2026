...
### ⚠️ مكتشفة وما انصلحت (قرار لك)
1. **ما فيه حد معدّل (rate limit) على `/api/listing-view` و`/api/contact-click`** — مُختبر فعليًا:
   أي شخص يقدر يرسل POST بلا حدود وبدون تسجيل دخول، ومعرّف الإعلان ظاهر بالرابط العام. يعني
   يقدر ينفخ (أو ينفخ لمنافس) عدّاد المشاهدات والتواصل. مهم لأن هذي الإحصائيات هي أساس فكرة
   تسييل بـ[PLAN.md](PLAN.md) §8.3 ("لوحة من شاف واهتم") وإشارة ثقة بـ[ROADMAP.md](ROADMAP.md)
   مرحلة 2 بند 9 ("دفتر تواصل علني"). الحل المقترح: تسجيل مرة وحدة لكل زائر/جلسة باليوم.
2. **`startCheckout` ينشئ charge بـTap قبل ما يحجز صفوف قاعدة البيانات** — لو فشل حفظ
   `subscriptions`/`payments`، يبقى charge يتيم عند Tap والمستخدم يشوف خطأ بدون طريقة استرجاع.
3. **رسالة خطأ مضللة بإضافة إعلان** — أي فشل insert (تصادم slug، حد يومي، مشكلة اتصال) يطلع
