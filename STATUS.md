...
| 16 | `interaction_rate_limit` | ⚠️ **غير مطبّقة بعد** — حد يومي على عدّادات المشاهدة/التواصل (تفاصيل تحت) |

*(رقم 8 محذوف عمدًا — كان بيانات أحياء مخترعة، اتشال قبل التطبيق)*

### ⚠️ خطوة يدوية معلّقة عليك: migration 16
لصّقها بـ**SQL Editor** بمشروع Supabase (نفس أسلوب كل الـmigrations السابقة — الـCLI هنا مو
مربوط بحساب Supabase البعيد فما أقدر أطبّقها من هنا). بعدها تأكد `INTERACTION_HASH_SECRET`
معبّى بـ`.env.local` (محلي) وبإعدادات النشر (Vercel لاحقًا) — بدونه الحد اليومي يشتغل لكن كل
الزوار يُعاملون كزائر واحد لأن السري يسقط لقيمة افتراضية محلية فقط.
