...
- `/search?q=` — بحث عربي مطبَّع (يطابق كيكه/كيكة) عبر `pg_trgm`

### تحسين محركات البحث (SEO)
- `sitemap.xml` ديناميكي (الفئات + البائعون المعتمدون + الإعلانات المنشورة)
- `robots.txt` يستبعد `/dashboard /admin /api /auth /login /search`
- `generateMetadata` لكل صفحة عامة — عنوان ووصف خاص بكل إعلان/بائع/فئة
