...
ثم الصق `00000000000058_newsletter.sql` (لصقة رابعة)، وأخيرًا `00000000000059_referral_followup.sql` (لصقة خامسة).
...
### 1.5 تحقق من صفحة Cron Jobs بلوحة Vercel
صار عندنا 3 مهام مجدولة بـ`vercel.json` (انتهاء الاشتراك، البريد، النشرة الأسبوعية). باقة Hobby سابقًا كانت تحدّد عدد الـcron jobs — النشر نجح بدون خطأ لكن تأكد بنفسك من https://vercel.com/salems-projects-514652d7/2026/settings/cron-jobs إن الثلاثة مفعّلة فعليًا ومالها حد يعطّل بعضها.
...
| 22 | إغلاق حلقة "رشّح مشروعًا" — مطابقة تلقائية عند انضمام المحل بنفسه + رابط واتساب مباشر لإدارة الترشيحات | `migrations/…59`, `app/dashboard/setup/actions.ts`, `app/admin/referrals` |
